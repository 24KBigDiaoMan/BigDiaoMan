...
static float const kBlurParameter = .5f;
...
- (id)init
{
    ...
    if (self) {
        AMBlurView *amblurView=[[AMBlurView alloc]initWithFrame:CGRectMake(0, 0, 320, 75)];
...
- (id)initWithStyle:(NZAlertStyle)style statusBarColor:(UIColor *)statusBarColor title:(NSString *)title message:(NSString *)message
{
    self = [self init];
...
//                                lineBreakMode:NSLineBreakByTruncatingTail].height;
    ...
    CGFloat height = [label.text boundingRectWithSize:CGSizeMake(CGRectGetWidth(label.frame), 9999)
